    ...
    def forward(self, x: torch.Tensor, res_connections: list = [], cat: bool = False):
        ...
        for i, module in enumerate(self):
            if i % 3 == 0 and not bool(res_connections):
                ...
                x = module(torch.cat((x, res_connections[i // 3]), dim=1)) if cat else module(
                    x + res_connections[i // 3])
            else:
                x = module(x)
...
class D_Net(torch.nn.Module):
    ...
    def __init__(self, in_resolution: tuple, activation=torch.nn.LeakyReLU, in_channels: int = 3, n_channels: int = 64,
                 kernel_size: int = 5):
